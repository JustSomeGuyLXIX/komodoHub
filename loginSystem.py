...
def register(): #aka register funtion
    ...
    DOB = bdayCal.get()
    #no gender attribute in database?

    if email!= '' and password != '' and fName != '' and lName != '' and confirmPass != '' and DOB != '':
        cursor.execute('SELECT email FROM users WHERE email=?', [email])
        if cursor.fetchone() is not None:
            messagebox.showerror('Error', 'Email already exists')
        else: #needs editing
            if password == confirmPass: #confirms passwords are the same
                ...
                cursor.execute('SELECT passID FROM passwords WHERE passHash=?', [passHash])
                passID = str(cursor.fetchone()) #this and an earlier lineprobably the cause of issues for login function
                #adds hashed password into database and stores ID
...
def login(): #aka login function
    ...
    if email != '' and password != '':
        ...
        if result:
            cursor.execute('SELECT passHash FROM passwords WHERE passID=?', [result])
            var = cursor.fetchone()
            if bcrypt.checkpw(password.encode('utf-8'), var[0]):
